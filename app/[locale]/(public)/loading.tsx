...
export default function PublicLoading() {
  return (
    <div className="container-shell animate-pulse py-16">
      <div className="bg-lime/60 mx-auto h-10 w-2/3 max-w-xl rounded-control" />
      <div className="bg-lime/40 mx-auto mt-4 h-5 w-1/2 max-w-md rounded-control" />
      <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <div
            key={index}
            className="bg-lime/20 h-48 rounded-card border border-border"
          />
        ))}
      </div>
    </div>
