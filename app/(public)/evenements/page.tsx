...
export const metadata: Metadata = {
  title: "Événements IA",
  description:
    "Découvrez les formations, ateliers et rencontres FancyVision et inscrivez-vous en ligne.",
};
...
export default async function EventsPage({
  ...
  const now = new Date();
  const view: EventFilters["view"] = params.view === "past" ? "past" : "upcoming";
  const audiences = [...new Set(allEvents.map((event) => event.audience).filter(Boolean))] as string[];
  const audience = audiences.includes(params.audience ?? "") ? params.audience : undefined;
  ...
  const selectedDay = selectedMonth && /^\d{1,2}$/.test(params.day ?? "")
    ? Number(params.day)
    : undefined;
  ...
  const events = audienceEvents.filter((event) => {
    if (selectedMonth && monthValue(event.startAt) !== selectedMonth) return false;
        ...
        <div className="mb-7 flex flex-col justify-between gap-5 border-b border-white/10 pb-6 sm:flex-row sm:items-end">
          <div>
            <span className="font-mono text-[11px] uppercase tracking-[0.14em] text-[#91abd6]">
              FancyVision Sessions
            </span>
            <h1 className="mt-2 text-3xl font-semibold tracking-[-0.035em]">Événements</h1>
          </div>
          <Link
            href="/rendez-vous"
          ...
          >
            <Plus className="size-4" /> Organiser un événement
          </Link>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_17.5rem]">
          <main>
            <div className="mb-6 rounded-2xl border border-white/10 bg-[#0c1b35] p-2.5">
              <div className="flex flex-wrap items-center gap-2">
                <Link
                  href={eventsHref(filters, { view: "upcoming", month: undefined, day: undefined })}
                ...
                <Link
                  href={eventsHref(filters, { view: "past", month: undefined, day: undefined })}
                ...
                {hasExtraFilters && (
                  <Link
                    href={eventsHref(filters, { audience: undefined, month: undefined, day: undefined })}
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-2 text-xs text-white/55">
                            <span className="inline-flex items-center gap-1.5">
                              ...
                              {event.endAt && (
                                <> – {new Intl.DateTimeFormat("fr-FR", { hour: "2-digit", minute: "2-digit" }).format(event.endAt)}</>
                              )}
                            </span>
                            <span className="text-[#ffd166]">Heure de Paris</span>
                          </div>
                          ...
                          <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-white/60">
                            <span className="inline-flex items-center gap-2">
                              <Users className="size-4" /> Par {event.host}
                            </span>
                            <span className="inline-flex items-center gap-2">
                          ...
                          <div className="mt-4 flex flex-wrap items-center gap-2">
                            ...
                            {event.capacity && (
                              <span className="text-xs text-white/40">
                                {Math.max(0, event.capacity - event._count.registrations)} places restantes
                              </span>
                            )}
                          </div>
                        </div>
          ...
          <aside className="lg:sticky lg:top-28 lg:self-start">
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-[#0c1b35] p-5">
              <p className="text-sm font-semibold">Une question avant de vous inscrire ?</p>
              <p className="mt-2 text-xs leading-5 text-white/50">
                Chaque inscription est enregistrée et notre équipe vous adresse les informations de connexion.
              </p>
              <Link href="/contact" className="mt-4 inline-flex text-xs font-semibold text-[#91abd6] hover:text-white">
                Contacter l’équipe →
              </Link>
            </div>
          </aside>
...
function MiniEventCalendar({
  ...
  const eventDays = new Set(
    events
      .filter((event) => event.startAt.getFullYear() === year && event.startAt.getMonth() === month)
    ...
    <div className="rounded-2xl border border-white/10 bg-[#0c1b35] p-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <Link
            href={eventsHref(filters, { month: monthValue(previousMonth), day: undefined })}
          ...
          <Link
            ...
            aria-label="Revenir au calendrier initial"
            className="grid size-8 place-items-center rounded-lg text-white/45 transition hover:bg-white/10 hover:text-white"
          >
            <span className="size-1.5 rounded-full bg-current" />
          </Link>
          <Link
            href={eventsHref(filters, { month: monthValue(nextMonth), day: undefined })}
      ...
      <div className="mt-4 grid grid-cols-7 gap-y-2 text-center text-xs">
        {["D", "L", "M", "M", "J", "V", "S"].map((day, index) => (
          <span key={`${day}-${index}`} className="font-semibold text-white/40">{day}</span>
        ))}
        ...
        {Array.from({ length: total }, (_, index) => index + 1).map((day) => {
          ...
          return hasEvent ? (
            <Link
              ...
              aria-label={`Filtrer les événements du ${day}`}
            ...
            >
              {day}
              {!active && <i className="absolute bottom-0.5 size-1 rounded-full bg-[#91abd6]" />}
            </Link>
          ) : (
            <span key={day} className="grid h-8 place-items-center text-white/30">
              {day}
            </span>
          );
        })}
